    ...
    <section style={{ marginBottom: 32 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20, marginBottom: 16 }}>
        <div>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 600, color: '#111', margin: 0 }}>
            Module performance at a glance
          </h2>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
        {modules.map((module) => {
          ...
          return (
            <Card key={module.label} style={{ padding: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
                ...
                <div>
                  <div style={{ fontSize: 15, fontWeight: 600, color: '#111' }}>{module.label}</div>
                  <div style={{ fontSize: 12, color: '#888', marginTop: 4 }}>{module.summary}</div>
                </div>
              </div>

              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#111' }}>{module.metric}</div>
                <div style={{ fontSize: 11, color: '#999' }}>{Math.round(module.progress * 100)}%</div>
              </div>
