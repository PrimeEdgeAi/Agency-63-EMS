import { useState } from 'react'
import { supabase } from '../../lib/supabase'
import type { AppUser } from '../../types'

interface LoginPageProps {
  onLogin: (user: AppUser) => void
}

export function LoginPage({ onLogin }: LoginPageProps) {
  const [loading, setLoading]   = useState(false)
  const [error, setError]       = useState('')
  const [btnHovered, setBtnHov] = useState(false)

  const handleGoogleLogin = async () => {
    setLoading(true)
    setError('')
    try {
      const { data, error: authError } = await supabase.auth.signInWithOAuth({ provider: 'google' })
      if (authError) {
        setError(authError.message)
        setLoading(false)
        return
      }
      if (data.user) {
        onLogin(data.user)
      } else {
        setError('Access denied. Your email is not on the approved list.')
        setLoading(false)
      }
    } catch {
      setError('Authentication failed. Please try again.')
      setLoading(false)
    }
  }

  return (
    <div style={{ minHeight: '100vh', display: 'flex', fontFamily: 'Georgia, serif' }}>

      {/* ── Left: Brand Logo Panel ───────────────────────────────────────── */}
      <div
        style={{
          flex: 1,
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          padding: '64px 56px',
          background: '#111',
        }}
      >
        <div
          style={{
            display: 'flex',
            flexDirection: 'column',
            alignItems: 'center',
            justifyContent: 'center',
            gap: 32,
            width: '100%',
            maxWidth: 520,
            textAlign: 'center',
          }}
        >
          <img
            src="./images.jpg"
            alt="EventPortal logo"
            style={{
              width: 110,
              height: 110,
              objectFit: 'cover',
              borderRadius: 24,
              border: '1px solid rgba(255,255,255,0.18)',
              boxShadow: '0 28px 60px rgba(0,0,0,0.25)',
            }}
          />

          <h1
            style={{
              color: 'white',
              fontSize: 56,
              fontWeight: 700,
              lineHeight: 1.05,
              margin: 0,
              letterSpacing: -1,
            }}
          >
            Every event<br />tells a story.
          </h1>
        </div>
      </div>

      {/* ── Right: Login Form ─────────────────────────────────────────────── */}
      <div
        style={{
          width: 480,
          display: 'flex',
          flexDirection: 'column',
          justifyContent: 'center',
          padding: '64px 56px',
          background: '#fafafa',
        }}
      >
        <div style={{ maxWidth: 340 }}>
          <h2
            style={{ fontSize: 28, fontWeight: 400, color: '#111', marginBottom: 8, letterSpacing: -0.5 }}
          >
            Welcome back
          </h2>
          <p style={{ color: '#999', fontSize: 15, marginBottom: 48, lineHeight: 1.7 }}>
            Sign in with your approved organisational Google account to continue.
          </p>

          {error && (
            <div
              style={{
                background: '#fff1f2',
                border: '1px solid #fecdd3',
                borderRadius: 10,
                padding: '12px 16px',
                marginBottom: 24,
                color: '#be123c',
                fontSize: 13,
                lineHeight: 1.5,
              }}
            >
              ⚠ {error}
            </div>
          )}

          <button
            onClick={handleGoogleLogin}
            disabled={loading}
            onMouseEnter={() => setBtnHov(true)}
            onMouseLeave={() => setBtnHov(false)}
            style={{
              width: '100%',
              padding: '14px 24px',
              borderRadius: 12,
              border: '1.5px solid #e5e5e5',
              background: btnHovered ? '#111' : 'white',
              color: btnHovered ? 'white' : '#111',
              fontSize: 15,
              fontWeight: 600,
              cursor: loading ? 'not-allowed' : 'pointer',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              gap: 12,
              transition: 'all 0.2s ease',
              fontFamily: 'Georgia, serif',
              opacity: loading ? 0.7 : 1,
            }}
          >
            {loading ? (
              <>
                <div
                  className="animate-spin-slow"
                  style={{
                    width: 18,
                    height: 18,
                    border: '2px solid rgba(180,180,180,0.4)',
                    borderTop: '2px solid #555',
                    borderRadius: '50%',
                  }}
                />
                Authenticating…
              </>
            ) : (
              <>
                <GoogleIcon inverted={btnHovered} />
                Continue with Google
              </>
            )}
          </button>

          <div style={{ marginTop: 48, paddingTop: 32, borderTop: '1px solid #f0f0f0' }}>
            <p style={{ fontSize: 12, color: '#ccc', lineHeight: 1.7 }}>
              Access is restricted to approved organisational emails. Contact your administrator
              if you need access granted.
            </p>
          </div>
        </div>
      </div>
    </div>
  )
}

function GoogleIcon({ inverted }: { inverted: boolean }) {
  const c = inverted ? 'white' : undefined
  return (
    <svg width="18" height="18" viewBox="0 0 24 24">
      <path
        fill={c ?? '#4285F4'}
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill={c ?? '#34A853'}
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill={c ?? '#FBBC05'}
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill={c ?? '#EA4335'}
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  )
}